...
subtitle: "ASV with Strong’s (Enhanced)"
...
<div class="hero">
  ...
  <p class="lead">Explore the ASV text with Strong’s numbers and enhanced glosses. Filter by book, chapter, or Strong’s code, and export selections for deeper study.</p>
  <div class="metrics">
    <div class="metric">
      <h4 id="m-verses">—</h4>
    ...
    <div class="metric">
      <h4 id="m-unique">—</h4>
      <small>Unique Strong’s codes</small>
    </div>
    <div class="metric">
      <h4 id="m-avg">—</h4>
...
> Tip: you can pre-filter via URL. For example, append `?q=H430` to search for a specific Strong’s code.
...
asvs_strongs_enhanced <- readr::read_csv(
  "GitHub/strongsBibleConcordence/asvs_strongs_enhanced.csv",
  ...
  htmlwidgets::onRender("
    function(el, x) {
      ...
      setTimeout(function(){
        ...
        if (search) search.placeholder = 'Search by text, book, or Strong\\'s code (e.g., H430)…';
      }, 0);
    }
  ")
```
...
## Strong’s Index (auto-generated)

This index is derived from the CSV on the fly—counts each Strong’s code across all verses and shows the most common first. Click a code to jump to an external lexicon entry.
...
<div class="section-note">
  <ul>
    ...
    <li><strong>Attribution:</strong> Scripture is ASV; Strong’s numbers &amp; glosses derived from your CSV.</li>
  </ul>
</div>
...
2. Knit to HTML in RStudio: **Knit ▶ index.html**.
3. Commit and push the generated `index.html` plus any referenced assets.
4. In your GitHub repo: **Settings ▶ Pages ▶ Build and deployment ▶ Source = Deploy from a branch**.
